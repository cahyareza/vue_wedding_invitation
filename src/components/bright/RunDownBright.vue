<template>
    <div v-if="acara.length != 0">
        <div :class="theme">
            <section class="low-opacity-bg-image-2" :style="{ 'background-image': 'url(' + portobackground.image2 + ')' }" style="min-height: 300px; background-position: center; position: relative; background-size: cover;
                object-fit: cover;">
                        ...
                        <div class="columns is-multiline">
                            <div class="column is-12 mb-2">
                                ...
                                <div v-if="piece.waktu_selesai_acara">
                                    <p class="subtitle3 is-size-7-tablet is-size-7-mobile mt-1 has-text-weight-bold mb-1"><font-awesome-icon icon="fa-solid fa-clock" /> pukul {{ waktu(piece.waktu_mulai_acara) }} - {{ waktu(piece.waktu_selesai_acara) }} {{ change_timezone(portofolio.timeZone) }}</p>
                                </div>
                                <div v-else>
                                    <p class="subtitle3 is-size-7-tablet is-size-7-mobile has-text-weight-bold mt-1 mb-1"><font-awesome-icon icon="fa-solid fa-clock" /> pukul {{ waktu(piece.waktu_mulai_acara) }} - selesai {{ change_timezone(portofolio.timeZone) }}</p>
                                </div>
                                <p class="subtitle3 is-size-7 has-text-weight-bold mt-1 mb-3"><font-awesome-icon icon="fa-solid fa-location-pin" /> {{ piece.tempat_acara }}</p>
